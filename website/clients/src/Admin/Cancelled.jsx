...
function Cancelled() {
  ...
  const handleUserCancel = async (cancel) => {
    try {
      setLoading(true);
      const response = await fetch(
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "status",
      key: "status",
      render: (status) => (status === 4 ? "Cancelled" : null),
    },
    {
    ...
    },
    {
      ...
      key: "name",
      render: (text, record) => record.user_id[0].username,
    },
    {
      title: "User Phone No",
      dataIndex: ["user_id", "phone"],
      key: "phone",
      render: (text, record) => record.user_id[0].phone,
    },
    {
      title: "Booking Hours",
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          <Button onClick={() => finishBooking(record)}><CloseOutlined/></Button>
        </div>
      ),
    },
  ];
